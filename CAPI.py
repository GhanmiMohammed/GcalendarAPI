...
print('cardentials has been set!')
...
def get_credentials():
    ...
    credential_path = 'credentials\client_secret_google_calendar.json'
...
def main():
 ...
 # This code is to look for all-day events in each calendar for the month of September
    ...
    start_date = datetime.datetime(2022, 11, 30, 00, 00, 00, 0).isoformat() + 'Z'
    end_date = datetime.datetime(2017, 12, 30, 23, 59, 59, 0).isoformat() + 'Z'
 
    getEvents(service, calendar_ids, start_date, end_date)

def getEvents(service, calendar_ids, start_date, end_date):
...
def setEvent(service, calendar_id, start_date, end_date):
    eventsResult = service.events().list(
            calendarId=calendar_id,
            timeMin=start_date,
            timeMax=end_date,
